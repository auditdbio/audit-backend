...
impl AuditRequestRepo {
    ...
    pub async fn create(&self, user: &AuditRequest) -> Result<bool, mongodb::error::Error> {
        self.0.create(user).await
    }

    pub async fn find(&self, id: ObjectId) -> Result<Option<AuditRequest>, mongodb::error::Error> {
    ...
    ) -> Result<Vec<AuditRequest>, mongodb::error::Error> {
