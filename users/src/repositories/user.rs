...
#[derive(Clone)]
pub struct UserRepo(Arc<dyn Repository<User, Error = mongodb::error::Error> + Send + Sync>);

impl UserRepo {
    pub fn new<T>(repo: T) -> Self
    where
        T: Repository<User, Error = mongodb::error::Error> + Send + Sync + 'static,
    {
        Self(Arc::new(repo))
    }

    pub async fn create(&self, user: &User) -> Result<bool, mongodb::error::Error> {
        self.0.create(user).await
    }

    pub async fn find(&self, id: ObjectId) -> Result<Option<User>, mongodb::error::Error> {
