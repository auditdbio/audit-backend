...
pub fn create(session: AuthSession) -> Result<String> {
    ...
    let jwt = encode(&Header::default(), &session, &key).is_signature()?.unwrap();
    Ok(jwt)
}
...
pub fn verify(jwt: &str) -> Result<Option<AuthSession>> {
    ...
    let jwt: Option<TokenData<AuthSession>> = decode::<AuthSession>(jwt, &key, &Validation::default()).is_signature()?;
