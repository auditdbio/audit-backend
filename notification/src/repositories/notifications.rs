...
impl NotificationsRepository {
    ...
    pub async fn insert(&self, notification: &Notification) -> error::Result<bool> {
        self.mongo.insert(notification).await
    }

    pub async fn get_unread(&self, user_id: &ObjectId) -> error::Result<Vec<Notification>> {
    ...
    pub async fn read(&self, id: ObjectId) -> error::Result<()> {
        self.mongo
            .collection
            .update_one(
                doc! {"id": id},
                doc! {"$set": {"inner.is_read": true}},
                None,
            )
            .await?;
        Ok(())
